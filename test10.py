...
# Set webcam size adjust automatically
...
def countdown_and_display_age(root, age, gender):
    ...
    if ad_url:
        # open the ad in full screen
        webbrowser.open(ad_url, new=2, autoraise=True)
    else:
        messagebox.showerror("No Ad Found!", "ไม่พอข้อมูลโฆษณาสำหรับกลุ่มอายุนี้")
...
def open_contact_developer():
    ...
    #Set the developer photo as the window icon
    ...
    photo_label.image = developer_photo  # Keep a reference to the image to prevent it from being garbage collected
...
def create_gui():
    ...
    Label(root, text="SMART AD SELECTOR", font=("Arial", 25), bg='#2E2E2E', fg='#FFFFFF').pack(pady=20)
    # Add picture banner to the GUI
    banner = Image.open("banner.png")
    banner = banner.resize((800, 300), Image.Resampling.LANCZOS)
    banner = ImageTk.PhotoImage(banner)
    banner_label = Label(root, image=banner, bg='#2E2E2E')
    banner_label.image = banner  # Keep a reference to the image to prevent it from being garbage collected
    banner_label.pack(pady=20)
    Label(root, text="AI-Powered Gender and Age Detection for Targeted Advertising", font=("Arial", 15), bg='#2E2E2E', fg='#FFFFFF').pack(pady=20)
    ...
    command=open_settings, font=("Arial", 10), bg='#4CAF50', fg='#FFFFFF', padx=5, pady=2.5)
    #settings button position left side of contact button
    settings_button.pack(pady=10)

    contact_button = Button(root, text="Developer",image=contact_icon, compound="left", command=open_contact_developer, font=("Arial", 12), bg='#FF5722', fg='#FFFFFF', padx=5, pady=2.5)
    #contact button position right side of settings button
